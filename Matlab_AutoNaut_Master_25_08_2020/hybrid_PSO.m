function [xBest,xBestVal] = hybrid_PSO(myFun,contInput,boolInput,seed,nParticles, maxIter)
...
%                    waypoint. 
    ...
    x0 = repmat(seed,1,nParticles) + 2*[randn(contInput,nParticles-1),zeros(contInput,1);
                                            zeros(boolInput,nParticles)];
    x = zeros(inputs,nParticles);
    v = zeros(contInput,nParticles);
    vb = zeros(boolInput,nParticles);
    x = x0;
    pBest = zeros(inputs,nParticles);
    pBestVal = 999999*ones(maxIter,nParticles);

    paramTreshCont = [0.5; 0.2; 0.4; 2; 1];
    paramTreshBool = [0.5; 0.5; 0.5; 0.5; 0.9];
